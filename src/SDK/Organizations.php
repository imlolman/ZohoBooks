...
class Organizations extends BaseClass
{
     ...
     * URL: https://{{ZOHO_BOOKS_DOMAIN}}/books/v3/organizations?organization_id={{ORGANIZATION_ID}}
     * Method: POST
     * Headers:
     * @param array $data = []
     */
    public function create_an_organization($data = [])
    {
        $url = $this->replaceVariables(
            "https://{{ZOHO_BOOKS_DOMAIN}}/books/v3/organizations?organization_id={{ORGANIZATION_ID}}"
        );
        $options = [];
        $options["headers"] = [];
        $options["form_params"] = [
    ...
    /**
     * Get organization
     * URL: https://{{ZOHO_BOOKS_DOMAIN}}/books/v3/organizations/${organization_id}?organization_id={{ORGANIZATION_ID}}
     * Method: GET
    ...
    {
        $url = $this->replaceVariables(
            "https://{{ZOHO_BOOKS_DOMAIN}}/books/v3/organizations/" .
                $organizationid .
